"""
Github: xichuanhit
https://github.com/xichuanhit/PerCode/blob/main/01-Electricity-Pre/LSTM-2.py
...
callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_loss', verbose=1, patience=1000),
             tf.keras.callbacks.ModelCheckpoint("model_lstm_02.hdf5", monitor='val_loss',
...
model_new = tf.keras.models.load_model("model_lstm_02.hdf5")
...
pred_train.to_csv("LSTM_2_train.csv",index=False)
pred_test.to_csv("LSTM_2_test.csv",index=False)
y_train.to_csv("LSTM_2_ytrain.csv",index=False)
