"""
Github: xichuanhit
https://github.com/xichuanhit/PerCode/edit/main/01-Electricity-Pre/BP-1.py
...
callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_loss', verbose=1, patience=1000),
             tf.keras.callbacks.ModelCheckpoint("model_bp_01.hdf5", monitor='val_loss',
...
model_load = tf.keras.models.load_model("model_bp_01.hdf5") # load the trained model
...
pred_train.to_csv("BP_1_train.csv",index=False)
pred_test.to_csv("BP_1_test.csv",index=False)
y_train.to_csv("BP_1_ytrain.csv",index=False)
